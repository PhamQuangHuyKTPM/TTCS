...
<head>
<meta charset="UTF-8">
<title>Điểm lớp ${nameClass } </title>
</head>
<body>
<div class="container mt-3"}>
	...
	<div class="container mt-3 shadow-lg p-3 mb-5 bg-white"
		style="background-color: white; padding: 20px 10px 20px 10px; border-radius: 20px">
		
		<h2>Quản lí điểm lớp ${nameClass } </h2>
		...
		<table class="table table-bordered table-striped">
			<thead>
				<tr>
					...
					<th>Trường</th>
					<th>Lớp</th>
					<th align="center">Điểm số 1</th>
					<th align="center">Điểm số 2</th>
					<th align="center">Điểm số 3</th>
					<th align="center">Điểm trung bình</th>
					<th align="center">Chỉnh sửa</th>
				</tr>
			</thead>
			<tbody id="myTable">
				<c:forEach var="list" items="${listTT }">
					<tr>
						<td>${list.hoten }</td>
						<td>${list.ngaysinh }</td>
						<td>${list.truong }</td>
						<td>${list.lop }</td>
						<td align="center">${list.diem1 }</td>
						<td align="center">${list.diem2 }</td>
						<td align="center">${list.diem3 }</td>
						<td align="center">${list.dtb }</td>
						<td align="center">
							<button type="button" class="btn btn-info">
								<a href="qltt?action=FormUpdate&id=${list.id }"><i
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-footer">
											<button type="button" class="btn btn-danger">
											<a href="qltt?action=delete&id=${list.id }&lop=${nameClass}"><i class="fa fa-trash-o" aria-hidden="true"></i> Xoá</a>
											</button>
										</div>
